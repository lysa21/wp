...
add_action('rest_api_init', function () {
	...
	register_rest_route('wp/v2', '/likes/(?P<id>\d+)', array(
		'methods' => array('POST'),
		'callback' => 'addLike',
	));
...
function addLike(WP_REST_Request $request)
{
	...
	$data = array("likes" => $updated_likes);

	echo json_encode($data);

}

function countLike(WP_REST_Request $request)
{
	$field_name = 'likes';
	// Get the current like number for the post
	$current_likes = get_field($field_name, $request['id']);
	$data = array("likes" => $current_likes);
